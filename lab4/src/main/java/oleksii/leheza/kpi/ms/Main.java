...
//        server.simulate(1000);
        ...
        printStatistic(server.getRequests(), server.getProcessed(), server.getFailed(),server);
    }

    public static void printStatistic(int requests, int processed, int failed, Server server) {
        double allProcessedTime = 0;
        for (Request request : server.getProcessedRequest()) {
            allProcessedTime += request.getBusyTime();
        }
        double averageProceedRequestTime = allProcessedTime / (double) server.getProcessedRequest().size();
        System.out.println("----Statistic----\n" + "All requests :" + requests + "\n" + "Processed requests: " + processed + "\n" + "Failed requests: " + failed + "\n" + "Average proceed request time: " + averageProceedRequestTime + "\n");
    }
}
